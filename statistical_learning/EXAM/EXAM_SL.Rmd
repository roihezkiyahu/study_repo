...
rotation - rotates the images

width, height shifts - which applies translation on the image to left/right, up/down

zoom - which zooms in/ out on an image

all of this transformation are rather reasonable by the sense hat applying them wont change the image much but they are still valid drown numbers. it doesn't really matter if you draw a number in the middle of the box or a bit shifted up/down/left/right, also the scale does not matter, it the same if you draw a small number or a larger number, and also the same applies for rotation
...
```
$S_{11}$ is scaled *100 so the we will see the relation.
...
for some reason i cant load the l1ce package so i will use glmnet.
...
```{r}
ridge_fit = glmnet(X_small, y_small, alpha = 0, lambda = seq(0, 0.5, by = 0.0001))
ridge_rss = colSums((y_small - predict(ridge_fit, s = seq(0, 0.5, by = 0.0001), newx = X_small))^2)
ridge_norm = colSums(coef(ridge_fit)[2:100,]^2)

lasso_fit = glmnet(X_small, y_small, alpha = 1, lambda = seq(0, 0.5, by = 0.0001))
lasso_rss = colSums((y_small - predict(lasso_fit, s = seq(0, 0.5, by = 0.0001), newx = X_small))^2)
...
on one hand it makes sense because if a lot of coefficients are small then the L2 loss should be lower (if they achieve a similar solution), but this got me to think and along with the recommendation to not use glmnet i took a closer look at the coefficient of the lasso, and saw non of them are 0!


```{r}
sum(abs(coef(lasso_fit))>0)
```

i dont know for sure why we get such a solution, i could not find a proper explanation in help(glmnet), but it might be that for very small values of $\lambda$ the glmnet treats it as 0 and thus yield a linear regression solution.

if i set lambda = 0.01 we get that some coefficients do become 0

```{r}
lasso_fit = glmnet(X_small, y_small, alpha = 1, lambda = 0.01)
lasso_rss = colSums((y_small - predict(lasso_fit, s = 0.01, newx = X_small))^2)
lasso_norm = sum(abs(coef(lasso_fit)[2:100]))
sum(abs(coef(lasso_fit))>0)
```
